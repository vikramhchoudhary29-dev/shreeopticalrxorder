...
export async function GET(request, { params }) {
  try {
    ...
    if (resource === 'customers') {
      ...
      return json(search
        ? await sql`SELECT * FROM customers WHERE name ILIKE ${`%${search}%`} OR mobile ILIKE ${`%${normalizeMobile(search)}%`} ORDER BY name LIMIT 100`
        : await sql`SELECT * FROM customers ORDER BY name`);
    }
