...
function CustomerPicker({ customers, value, onChange }) {
  ...
  const list = useMemo(() => {
    ...
    const source = q ? (remote.length ? remote : customers) : customers;
    return source
      ...
      <div className="picker-wrap">
        ...
        {open && (
          <div className="picker-menu">
            {searching ? <div className="picker-empty">Searching customers…</div> : list.length ? list.map((customer) => (
              <button
                type="button"
                key={customer.id}
